...
export const cartAdd = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ success: false, message: "Please login to add to cart" });
    }
    ...
    if (!cartAddProgress.success) {
      return res
        .status(500)
        .json({ success: false, message: cartAddProgress.message });
    }

    return res
      .status(200)
      .json({ success: true, message: cartAddProgress.message });
  } catch (error) {
