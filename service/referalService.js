...
export const applyReferralReward = async (newUser, refToken) => {
  try {
    ...
    let referrer = null;
    let referalEntry = await referalModel.findOne({
      token: refToken,
      used: false,
    });
    ...
    const refereeCredit = await creditWallet(
      newUser._id,
      REFEREE_REWARD,
      `Referral sign-up bonus from ${referrer.fullName}`,
    );
    if (!refereeCredit.success) {
      console.log(
        "Referrer credited but referee credit failed — manual reconciliation needed",
      );
    }
    ...
    console.log(
      `Referral reward applied: referrer ${referrer.email} +${REFERRER_REWARD}, new user +${REFEREE_REWARD}`,
    );
  } catch (error) {
